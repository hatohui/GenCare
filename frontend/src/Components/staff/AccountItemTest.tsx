...
interface AccountItemTestProps {
	item: Account
}

const AccountItemTest = ({ item }: AccountItemTestProps) => {
	...
	const handleClick = () => {
		setIsLoading(true)
		router.push(`/dashboard/tests/${item.id}`)
	}
	...
	return (
        ...
        `}
				...
				role='button'
				tabIndex={0}
				aria-label={`Xem chi tiết xét nghiệm cho ${item.firstName} ${item.lastName}`}
			>
				{/* Loading Overlay */}
				{isLoading && (
				...
				<div className='mt-auto'>
					<div className='flex items-center justify-center gap-2 text-accent group-hover:text-main transition-colors'>
						...
						<span className='text-sm font-medium'>Xem xét nghiệm</span>
					</div>
...
export default AccountItemTest
