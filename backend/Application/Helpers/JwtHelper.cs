...
public static class JwtHelper
{
    ...
    )
    {
        ...
        {
            new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString()), // Unique Identifier cho refresh token
        ...
        var refreshTokenString = new JwtSecurityTokenHandler().WriteToken(refreshTokenJwt);
    ...
    public static Guid ExtractAccountIdFromToken(string token)
    {
        var principal = ValidateToken(token);
        ...
        if (string.IsNullOrEmpty(accountIdString))
        {
            throw new AppException(401, "Account ID not found in the token.");
        }
        ...
        throw new AppException(401, "Account ID is not in a valid format.");
    }
